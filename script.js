...
function createProductCard(product) {
    ...
    if (categoryName === 'valorant') {
        ...
        if (['low', 'medium', 'high'].includes(level)) {
            imgSrc = level === 'high' ? 'rp3.png' : level === 'medium' ? 'rp2.png' : 'rp.png';
        } else {
    ...
    card.innerHTML = `
        ...
        <div class="product-info">
            ...
            ${product.description ? `<div class="product-description" style="font-size: 11px; color: #888; margin: 6px 0; line-height: 1.4; word-wrap: break-word; overflow-wrap: break-word;">${product.description}</div>` : ''}
            <div class="price">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Kategori kartları: ayrı görünüm aç
  const categoryCards = document.querySelectorAll('.categories .category-card');
  const categoryMap = [
    { key: 'steam', label: 'Steam Oyunları' },
    { key: 'aaa', label: 'AAA Oyunlar' },
    { key: 'currency', label: 'Oyun İçi Para' },
    { key: 'giftcards', label: 'Hediye Kartları' }
  ];
  categoryCards.forEach((card, idx) => {
    card.style.cursor = 'pointer';
    card.addEventListener('click', (e) => {
      e.preventDefault();
      const cat = categoryMap[idx] || categoryMap[0];
    ...
    link.addEventListener('click', (e) => {
      ...
      switch (sub) {
        ...
        case 'steam-game-code':
          openCategoryPage('steam', 'Steam Oyun Kodları', { nameIncludes: 'Oyun Kodu' });
          break;
        case 'steam-random-game-code':
          openCategoryPage('steam', 'Steam Rastgele Kodlar', { nameIncludes: 'Rastgele' });
          break;
        default:
