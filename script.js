...
function setupImmediateUpdateListeners() {
    // Listen for BroadcastChannel messages for featured products
    if (typeof BroadcastChannel !== 'undefined') {
        ...
        featuredChannel.addEventListener('message', (event) => {
            if (event.data.type === 'update') {
                console.log('Received immediate featured products update signal from admin panel');
                setTimeout(() => {
        ...
        // Listen for BroadcastChannel messages for general products
        const productsChannel = new BroadcastChannel('productsChannel');
        productsChannel.addEventListener('message', (event) => {
            if (event.data.type === 'update') {
                console.log('Received immediate products update signal from admin panel');
                setTimeout(() => {
                    loadFeaturedProducts().then(() => {
    ...
    let lastProductsUpdateTime = localStorage.getItem('productsUpdate') || '0';
    
    setInterval(() => {
        // Check for featured products updates
        ...
        if (currentFeaturedUpdateTime !== lastFeaturedUpdateTime && currentFeaturedUpdateTime !== '0') {
            console.log('Detected featured products localStorage update signal');
        ...
        // Check for general products updates
        const currentProductsUpdateTime = localStorage.getItem('productsUpdate') || '0';
        if (currentProductsUpdateTime !== lastProductsUpdateTime && currentProductsUpdateTime !== '0') {
            console.log('Detected products localStorage update signal');
            lastProductsUpdateTime = currentProductsUpdateTime;
            setTimeout(() => {
                loadFeaturedProducts().then(() => {
                    showNotification('Ürünler güncellendi', 'success', 2000);
                });
...
function createProductCard(product) {
  ...
  card.innerHTML = `
    <div class="product-image">
      <img src="https://via.placeholder.com/300x200/2c3e50/ffffff?text=${encodeURIComponent(product.name)}" alt="${product.name}">
    </div>
    <div class="product-info">
      <h3 class="product-title">${product.name}</h3>
      <p class="product-description">${product.description || 'Açıklama yok'}</p>
      <div class="product-meta">
        <span class="product-category">${product.category || 'Genel'}</span>
        <span class="product-price">${product.price || 0} ${product.currency || 'TL'}</span>
      </div>
      <div class="product-actions">
        <button class="btn primary" onclick="addToCart(${product.id})">
          <i class="fas fa-shopping-cart"></i> Sepete Ekle
        </button>
        <button class="btn favorite-btn" onclick="toggleFavorite(${product.id})">
          <i class="fas fa-heart"></i>
        </button>
      </div>
    </div>
...
async function validateCartCoupon() {
  ...
  try {
    ...
    const response = await fetch('/api/coupons/validate', {
      ...
      body: JSON.stringify({
        code: couponCode,
        order_amount: cartTotal * 100 // Convert to kuruş
      })
    });
    ...
    if (data.ok) {
      ...
      showCouponResult('success', `Kupon uygulandı! ${data.discount_amount}₺ indirim`);
      if (window.shoppingCart) {
        window.shoppingCart.updateCartWithDiscount(data.discount_amount);
      }
    } else {
      showCouponResult('error', data.error || 'Kupon kodu geçersiz');
    }
