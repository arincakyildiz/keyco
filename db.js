...
if (userCount === 0) {
  ...
  // Create custom admin user from environment
  const customAdminName = process.env.CUSTOM_ADMIN_NAME || 'CustomAdmin';
  const customAdminEmail = process.env.CUSTOM_ADMIN_EMAIL || 'custom@keyco.local';
  const customAdminPass = process.env.CUSTOM_ADMIN_PASSWORD || 'admin123';
