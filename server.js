...
app.use(helmet.contentSecurityPolicy({
    ...
    directives: {
        ...
        // Allow inline scripts during dev to avoid CSP blocks from inline handlers
        "script-src": ["'self'", "'unsafe-inline'"],
        // Allow event handler attributes during dev (e.g., onclick) – remove in prod
...
app.get('/api/admin/products', requireAuth, requireAdmin, (req, res) => {
    try {
        ...
        const query = `
            SELECT id, name, slug, price, currency, category, platform, package_level, description, created_at
            FROM products
            ORDER BY id DESC
        `;
...
app.post('/api/products', requireAuth, requireAdmin, [
    ...
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ ok: false, errors: errors.array().map(e => e.param) });
    const { name, slug, description, price, currency = 'TRY', category, platform, package_level } = req.body;
    try {
        // Insert into products table
        const info = db.prepare('INSERT INTO products (name, slug, description, price, currency, category, platform, package_level) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
            .run(name, slug, description || null, price, currency, category || null, platform || null, package_level || null);
        const product = db.prepare('SELECT * FROM products WHERE id = ?').get(info.lastInsertRowid);
        
        // Automatically add to featured products
        try {
            ...
            db.prepare('INSERT INTO featured_products (name, platform, price, discount, badge, icon, display_order) VALUES (?, ?, ?, ?, ?, ?, ?)')
                .run(name, platform || 'Steam', priceInTL, 0, 'new', 'fas fa-plus', nextOrder);
        } catch (featuredError) {
...
app.put('/api/products/:id', requireAuth, requireAdmin, [
    ...
    const { name = cur.name, slug = cur.slug, description = cur.description, price = cur.price, currency = cur.currency, category = cur.category } = req.body || {};
    try {
        db.prepare('UPDATE products SET name=?, slug=?, description=?, price=?, currency=?, category=? WHERE id = ?')
          .run(name, slug, description, price, currency, category, id);
...
app.get('/api/favorites', requireAuth, (req, res) => {
    const rows = db.prepare(`
        SELECT p.id, p.name, p.slug, p.description, p.price, p.currency, p.category, p.platform, p.package_level, f.created_at AS favorited_at
        FROM favorites f
...
app.get('/api/cart', requireAuth, (req, res) => {
    const rows = db.prepare(`
        SELECT p.id, p.name, p.slug, p.description, p.price, p.currency, p.category, p.platform, p.package_level, c.quantity, c.created_at AS added_at
        FROM cart_items c
...
app.post('/api/coupons/validate', requireAuth, (req, res) => {
    ...
    try {
        ...
        if (!coupon) {
            return res.status(404).json({ ok: false, error: 'invalid_coupon' });
        }
        ...
        if (alreadyUsed) {
            return res.status(400).json({ ok: false, error: 'coupon_already_used' });
        }
        
        let discount_amount = 0;
        if (coupon.type === 'percentage') {
            discount_amount = Math.floor(order_amount * (coupon.value / 100));
        } else {
